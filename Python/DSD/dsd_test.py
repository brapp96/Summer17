...
data = np.loadtxt('outputfile.DSD1', skiprows=1, usecols=(np.linspace(1,10,10,
    dtype=int)));
vertices = np.loadtxt('outputfile.DSD1', skiprows=1, usecols=(0), dtype=str)
...
spectral = sc.SpectralClustering(n_clusters = 3, affinity='precomputed')
spectral.fit(data)
